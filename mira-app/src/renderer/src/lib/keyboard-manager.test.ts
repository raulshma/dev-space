...
describe('KeyboardManager', () => {
  ...
  it('should initialize with default shortcuts', () => {
    ...
    expect(allShortcuts.get('command-palette:open')).toBe(DEFAULT_SHORTCUTS['command-palette:open'])
    expect(allShortcuts.get('zen-mode:toggle')).toBe(DEFAULT_SHORTCUTS['zen-mode:toggle'])
  })
  ...
  it('should load shortcuts from database', () => {
    ...
    expect(manager.getBinding('sidebar:toggle')).toBe(DEFAULT_SHORTCUTS['sidebar:toggle'])
  })
  ...
  it('should get binding for an action', () => {
    ...
    expect(manager.getBinding('command-palette:open')).toBe('Mod+P')
    expect(manager.getBinding('zen-mode:toggle')).toBe(DEFAULT_SHORTCUTS['zen-mode:toggle'])
  })

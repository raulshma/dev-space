...
export function CommandLibrary({ projectId }: CommandLibraryProps) {
  ...
  const categories = commandsByCategory ? Object.keys(commandsByCategory).sort() : []
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <div className="flex items-center justify-between border-b border-neutral-200 px-3 py-2">
        <h3 className="text-sm font-semibold text-neutral-900">Command Library</h3>
        <button
      ...
      {showAddForm && (
        <div className="border-b border-neutral-200 bg-neutral-50 p-3">
          <form onSubmit={handleAddCommand} className="flex flex-col gap-2">
            <div>
              <label htmlFor="command-name" className="mb-1 block text-xs font-medium text-neutral-700">
                Name
              </label>
              <input
                id="command-name"
            ...
            <div>
              <label htmlFor="command-text" className="mb-1 block text-xs font-medium text-neutral-700">
                Command
              </label>
              <input
                id="command-text"
            ...
            <div>
              <label htmlFor="command-category" className="mb-1 block text-xs font-medium text-neutral-700">
                Category (optional)
              </label>
              <input
                id="command-category"
            ...
            <div key={category} className="border-b border-neutral-100">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-neutral-500">{categoryCommands.length}</span>
                  <svg
                  ...
                  {categoryCommands.map((command) => (
                    ...
                    >
                      <div className="flex w-full items-center justify-between">
                        <span className="font-medium text-neutral-900">{command.name}</span>
                        {command.isCustom && (
                      ...
                      <code className="text-xs text-neutral-600">{command.command}</code>
                    </button>
                  ))}
                </div>
              )}

...
export function ApiKeyManager(): React.JSX.Element {
  ...
  const loadProviderStatus = async (): Promise<void> => {
    try {
      ...
      for (const provider of PROVIDERS) {
        const response = await window.api.keychain.has({ provider: provider.provider })
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Header */}
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-neutral-100">API Key Management</h3>
        <p className="text-sm text-neutral-400">
          Configure API keys for AI providers. Keys are stored securely in your OS keychain.
        </p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="space-y-4">
          {PROVIDERS.map((config) => {
            ...
            return (
              <div key={config.provider} className="p-4 border border-neutral-700 rounded-lg">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h4 className="text-sm font-medium text-neutral-100">{config.name}</h4>
                      {isConfigured && (
                    ...
                    <p className="mt-1 text-xs text-neutral-400">{config.description}</p>
      ...
      <div className="mt-6 pt-4 border-t border-neutral-700">
        <div className="flex items-start gap-2 text-xs text-neutral-400">
          ...
          <p>
            API keys are stored securely in your operating system&apos;s keychain and are never sent
            anywhere except to the respective AI provider.
          </p>
        </div>
      </div>
    </div>

...
export class AutoModeService extends EventEmitter {
  ...
  async approvePlan(
    ...
    if (updatedFeature) {
      ...
      this.resolvePlanApproval(featureId, true)
    }
  ...
  private async executeFeatureAsync(
    ...
    try {
      ...
      if (errorInfo.type === 'rate_limit') {
        await this.handleRateLimit(projectPath, feature.id, toProviderErrorInfo(errorInfo))
        return
      }
  ...
  private async handleRateLimit(
    ...
    this.emit('rateLimitWait', projectPath, waitUntil.toISOString(), waitSeconds)
    this.emitStateChanged(projectPath)
